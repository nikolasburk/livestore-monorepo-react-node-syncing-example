...
import { tables, events, Filter } from '@livestore-monorepo/shared/src/schema';
import { queryDb } from "@livestore/livestore";

function App() {
  const { store } = useStore();

  const [{ input, filter }, setUiState, , uiState$] = store.useClientDocument(
    tables.uiState
  );
  ...
  const updatedInput = (input: string) =>
    setUiState((state) => ({ ...state, input }));
  const updatedFilter = (filter: Filter) =>
    setUiState((state) => ({ ...state, filter }));
  // const [input, updatedInput] = useState('')
  // const [filter, updatedFilter] = useState<'All' | 'Active' | 'Completed'>('All')

  const addTodo = () => {
    if (input.trim()) {
      store.commit(events.todoCreated({ id: crypto.randomUUID(), text: input }));
      updatedInput("");
    }
  };
        ...
        <div className="flex gap-1 mb-4 border-b border-gray-200 justify-center">
          {(["All", "Active", "Completed"] as const).map((tab) => (
            <button
              ...
              className={`px-5 py-2.5 text-sm bg-transparent border-0 -mb-px cursor-pointer outline-none transition-colors ${
                filter === tab
            ...
            >
              <div className="flex items-center gap-3 flex-1">
                <input
                  ...
                  onChange={() => toggleTodo(todo.id as string, todo.completed)}
              ...
              <button
                onClick={() => deleteTodo(todo.id as string, new Date())}
